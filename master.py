...
def solve_master_problem(
    *,
    ...
    if K > 0:
        ...
        # (1.3)  y^r ≤ A_r P̄^r_{w,h,t}  (A_r = n_turbines, operación)
        m.addConstrs(
            (
                y_r[w, h, t, k] <= n_turb[w] * SCENARIOS[k].p_wind[w, h, t]
    ...
    wind_availability = np.broadcast_to(n_turb[:, None], (W, T)).astype(float).copy()
